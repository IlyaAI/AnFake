...
namespace AnFake.Core
{
	...
	public static class BuildServer
	{
		...
		/// </code>
		/// </example>
		public static Uri ExposeArtifact(FileItem file, string type = ArtifactType.Other)
		{
			if (file == null)
				throw new ArgumentException("BuildServer.ExposeArtifact(file[, type]): file must not be null");
			if (type == null)
				throw new ArgumentException("BuildServer.ExposeArtifact(file[, type]): type must not be null");
		...
		public static Uri ExposeArtifact(FolderItem folder, string type = ArtifactType.Other)
		{
			if (folder == null)
				throw new ArgumentException("BuildServer.ExposeArtifact(folder[, type]): folder must not be null");
			if (type == null)
				throw new ArgumentException("BuildServer.ExposeArtifact(folder[, type]): type must not be null");
		...
		/// <returns>URI of exposed artifact (not null)</returns>
		/// <seealso cref="ExposeArtifact(AnFake.Core.FileItem,String)"/>
		public static Uri ExposeArtifact(string name, string content, Encoding encoding, string type = ArtifactType.Other)
		{
			...
			if (type == null)
				throw new ArgumentException("BuildServer.ExposeArtifact(name, content, encoding[, type]): type must not be null");
		...
		/// <returns>URI of exposed artifact (not null)</returns>
		/// <seealso cref="ExposeArtifact(AnFake.Core.FileItem,String)"/>
		public static void ExposeArtifacts(FileSet files, string type = ArtifactType.Deliverables)
		{
			if (files == null)
				throw new ArgumentException("BuildServer.ExposeArtifacts(files[, type]): files must not be null");
			if (type == null)
				throw new ArgumentException("BuildServer.ExposeArtifacts(files[, type]): type must not be null");
